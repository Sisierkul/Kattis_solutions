...
for i in range (n):
    ...
    for j in range (m):
        H = grid[i][j]
        if H =='.':
            g.addEdge(0, i*m+j+1, b)
        else:
            g.addEdge(i*m+j+1, n*m+1, b)
        if j != m-1:
            g.addEdge(i*m+j+1, i*m+j+2, a)
            g.addEdge(i*m+j+2, i*m+j+1, a)
        if i != n-1:
            g.addEdge(mid*m+j+1, i*m+j+1, a)
            g.addEdge(i*m+j+1, mid*m+j+1, a)

print(g.DinicMaxflow(0, n*m+1))
